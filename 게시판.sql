...
drop table users;
insert into users(uid, upass, uname) values('blue', 'qmffn', '김블루');
insert into users(uid, upass, uname) values('sky', 'gksmf', '강하늘');
insert into users(uid, upass, uname) values('purple', 'vjvmf', '박보라');
insert into users(uid, upass, uname) values('jun', 'ho', '이준호');
insert into users(uid, upass, uname) values('code', 'master', '최코드');

select * from users;

update users set address1 = '인천 서구 경서동', phone='010-1234-5678', address2='101동 206호' where uid = 'purple';
...
insert into bbs (title, contents, writer)
select title, contents, writer from bbs;

select * from view_bbs WHERE title LIKE '%리%' order by bid desc;
...
insert into comments(bid, writer, contents) values(228, 'blue', 'Node.js의 장점은 무엇일까요? 앞으로 취업 전망은 괜찮을까요?');
insert into comments(bid, writer, contents) values(228, 'sky', '이 문서는 React Compiler가 무엇인지 알아보기 위해 여러 자료들을 수집해서 정리해 놓은 것입니다.');
insert into comments(bid, writer, contents) values(228, 'jun', '과거에 Java Script는 프론트 엔드 분야에서만 실행되는 언어로 취급되었지만 Node.js라는 런타임 환경이 등장함에 따라 프론트엔드와 백엔드 모두를 아우르는 언어가 되었는데요.');
insert into comments(bid, writer, contents) values(228, 'code', 'Node.js의 장점은 무엇일까요? 앞으로 취업 전망은 괜찮을까요?');
