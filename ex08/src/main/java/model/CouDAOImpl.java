...
//				System.out.println(cou.toString());
	...
	public void update(CouVO vo) {
		// TODO Auto-generated method stub
		try {
			String sql = "UPDATE courses SET lname=?, hours=?, room=?, instructor=?, persons=?, capacity=? WHERE lcode=?";
			PreparedStatement pstmt = con.prepareStatement(sql);
			pstmt.setString(1, vo.getLname());
			pstmt.setInt(2, vo.getHours());
			pstmt.setString(3, vo.getRoom());
			pstmt.setString(4, vo.getInstructor());
			pstmt.setInt(5, vo.getPersons());
			pstmt.setInt(6, vo.getCapacity());
			pstmt.setString(7, vo.getLcode());
			pstmt.execute();
		} catch (Exception e) {
			// TODO: handle exception
